...
export default class ProductModel{
    ...
    return products.filter((p) => {
        ...
        const isCategoryMatch = !category || p.category === category;
    ...
    static rateProduct(userId, productId, rating){
        ...
        if(!product){
            throw new ApplicationErrorError("Product not found");
        }

        //Check if there are any ratings and if not, add the ratings array to the product obj
        if(!product.ratings){
            product.ratings = [];
            ...
            //If there exists ratings update the ratings
            ...
            if(existingRatingIndex >= 0){
                ...
                //if no existing ratings, we add new rating
                product.ratings.push({
                    userId: userId,
                    rating: rating
                });
